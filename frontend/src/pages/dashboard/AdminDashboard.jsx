...
function KPICard({ title, value, unit, subtitle, icon, color, trend, onClick }) {
    return (
        <Card
            sx={{
                height: '100%',
                ...
                position: 'relative',
                overflow: 'hidden',
                '&:hover': onClick ? { transform: 'translateY(-4px)', boxShadow: 6, borderColor: color } : {},
                '&::before': {
                    content: '""',
                    position: 'absolute',
                    top: 0,
        ...
        >
            <CardContent sx={{ pt: 2.5 }}>
                <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    ...
                    <Avatar sx={{ bgcolor: `${color}18`, color, width: 52, height: 52, ml: 1 }}>
                        {icon}
                    </Avatar>
                </Box>
                {trend !== undefined && (
...
function SectionCard({ title, subtitle, children, action, minHeight }) {
    return (
        <Paper elevation={2} sx={{ p: 3, height: '100%', minHeight, borderRadius: 2 }}>
...
export default function AdminDashboard() {
    ...
    const loadDashboardData = async () => {
        setLoading(true);
        try {
            const [statsData, kpisData, notifsData, conflitsData] = await Promise.all([
    ...
    const mainCards = [
        ...
        },
        {
            ...
            trend: stats?.total_salles ? (stats.salles_utilisees / stats.total_salles) * 100 : 0,
        ...
        },
        {
            ...
            subtitle: conflits.length > 0 ? '⚠ Action requise' : '✓ Aucun conflit actif',
            ...
            <Box sx={{ flexGrow: 1 }}>
                ...
                <Paper elevation={2} sx={{ p: 2.5, mb: 3, borderRadius: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                            <Button variant="outlined" startIcon={<Refresh />} onClick={loadDashboardData} size="small">
                                Actualiser
                            </Button>
                            <Button
                                variant="contained"
                    ...
                    {!kpis && !loading ? (
                        <Box sx={{ textAlign: 'center', py: 3 }}>
                            <Typography color="text.secondary">
                                Aucune donnée disponible. Créez des affectations pour voir les KPIs.
                            </Typography>
                        </Box>
                    ) : (
                                ...
                                <Grid container spacing={3} alignItems="center">
                                    <Grid item xs={12} md={5}>
                                        <ResponsiveContainer width="100%" height={260}>
                                            <PieChart>
                                                <Pie
                                                    data={filiereData.filter((f) => f.nombre_seances > 0)}
                                    ...
                                    <Grid item xs={12} md={7}>
                                        ...
                                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                            {filiereData.slice(0, 8).map((f, idx) => (
                                                <Box key={f.nom}>
                                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                                        ...
                                                        <Box sx={{ display: 'flex', gap: 1, ml: 1 }}>
                                                            <Chip label={`${f.nombre_seances} séances`} size="small" />
                                                            <Chip
                                                                label={`${f.nombre_heures}h`}
                                                                size="small"
                                                                sx={{ bgcolor: `${PALETTE[idx % PALETTE.length]}20`, color: PALETTE[idx % PALETTE.length] }}
                                                            />
                                                        </Box>
                                                    </Box>
                                                    <LinearProgress
                                                        variant="determinate"
                                                        value={
                                                            filiereData[0]?.nombre_seances > 0
                                                                ? (f.nombre_seances / filiereData[0].nombre_seances) * 100
                                                                : 0
                                                        }
                                                        sx={{
                                                            bgcolor: `${PALETTE[idx % PALETTE.length]}20`,
                                                            '& .MuiLinearProgress-bar': { bgcolor: PALETTE[idx % PALETTE.length] },
                                                        }}
                                                    />
                                                </Box>
                                            ))}
                        ...
                        <Box>
                            {creneauxData.length === 0 ? (
                                ...
                                <ResponsiveContainer width="100%" height={280}>
                                    <BarChart data={creneauxData} margin={{ top: 10, right: 20, left: 0, bottom: 60 }}>
                                        ...
                                        <ReTooltip
                                            formatter={(v) => [`${v} affectation(s)`, 'Demandes']}
                                            labelFormatter={(l) => `Créneau: ${l}`}
                                        />
                                        <Bar dataKey="count" radius={[6, 6, 0, 0]} fill="#1a3a8f">
                    ...
                    {tabChart === 2 && (
                        <Box>
                            {sallesGraphData.length === 0 ? (
                                <Box sx={{ textAlign: 'center', py: 4 }}>
                                    <Room sx={{ fontSize: 48, color: 'text.disabled', mb: 1 }} />
                                ...
                                <ResponsiveContainer width="100%" height={280}>
                                    ...
                                    >
                                        ...
                                        <XAxis
                                            ...
                                            tick={{ fontSize: 11 }}
                                        />
                                        <YAxis dataKey="nom" type="category" tick={{ fontSize: 11 }} width={75} />
                                        <ReTooltip
                                            formatter={(v, n, p) => [`${v}% (${p.payload.heures}h)`, "Taux d'occupation"]}
                                        />
                                        <Bar dataKey="taux" radius={[0, 6, 6, 0]}>
                                            {sallesGraphData.map((entry, idx) => (
                                                <Cell
                                                    key={idx}
                                                    fill={
                                                        entry.taux >= 70 ? '#c62828' :
                                                        entry.taux >= 40 ? '#e8a020' :
                                                        '#2e7d32'
                                                    }
                                                />
                                            ))}
                            ...
                            <Box sx={{ display: 'flex', gap: 2, mt: 1, flexWrap: 'wrap' }}>
                                {[
                                    { color: '#2e7d32', label: '< 40% — Faible' },
                                    { color: '#e8a020', label: '40-70% — Moyen' },
                                    { color: '#c62828', label: '> 70% — Élevé' },
                                ].map((l) => (
                ...
                <Grid container spacing={3}>
                        ...
                        <SectionCard
                            ...
                            action={
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    <IconButton size="small" onClick={() => navigate('/notifications')} color="primary">
                                        <Notifications />
                                    </IconButton>
                                </Box>
                            }
                                                    ...
                                                    primary={
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                            ...
                                                            <Chip
                                                                ...
                                                                size="small"
                                                                color={
                                                                    notif.type_notification === 'error' ? 'error' :
                                                                    notif.type_notification === 'warning' ? 'warning' : 'primary'
                                                                }
                                                                variant="outlined"
                                                            />
                                                        </Box>
                    ...
                    <Grid item xs={12} md={6}>
                        <SectionCard
                            title="Conflits non résolus"
                            subtitle={conflits.length > 0 ? 'Intervention requise' : 'Tout est en ordre'}
                            minHeight={320}
                            action={
                                <Chip
                                    label={conflits.length}
                                    color={conflits.length > 0 ? 'error' : 'success'}
                                    size="small"
                                    icon={conflits.length > 0 ? <Warning fontSize="small" /> : <CheckCircle fontSize="small" />}
                                />
                            }
                        >
                            {conflits.length > 0 ? (
                                ...
                                <Box sx={{ textAlign: 'center', py: 4 }}>
                                    <CheckCircle sx={{ fontSize: 48, color: 'success.main', mb: 1 }} />
                                    <Typography variant="body2" color="text.secondary">
                                        Aucun conflit à résoudre — emploi du temps cohérent
                                    </Typography>
                                </Box>
                            )}
                        </SectionCard>
