...
const USERS = {
    ...
    enseignant: { email: 'alain.bennis0@hestim.ma',  password: 'password123', role: 'enseignant' },
    etudiant:   { email: 'aya.benali0@hestim.ma',    password: 'password123', role: 'etudiant' },
...
async function login(page, role = 'admin') {
    ...
    const fonctionMap = { admin: 'Administrateur', enseignant: 'Professeur', etudiant: 'Étudiant' };
    const select = page.locator('select, [role="combobox"]').filter({ hasText: /fonction|rôle/i }).first();
 ...
 */
async function loginViaToken(page, role = 'admin') {
    const user = USERS[role];

    // Appel direct à l'API login pour récupérer le token
    const response = await page.request.post('http://localhost:5000/api/auth/login', {
        data: { email: user.email, password: user.password },
    });

    if (!response.ok()) {
        throw new Error(`Login API failed: ${response.status()} — vérifier que le seed est lancé`);
    }
...
async function logout(page) {
    ...
    const avatar = page.locator('[aria-label*="compte"], [aria-label*="profil"], .MuiAvatar-root').first();
